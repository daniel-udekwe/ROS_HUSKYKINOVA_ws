...
class messageConverter:
    def __init__(self):
        ...
        #self.ArmOrientation = rospy.Subscriber("/myGen3/armorientation", Twist, self.base_feedback_callback)
    ...
    def base_feedback_callback(self, data):
        ...
        rospy.loginfo(Position_Message.linear.z)
        self.Kinova_Position.publish(Position_Message)
        
        with open("/home/administrator/ws_daniel/x_posData.txt", "a") as log_file:
            #log_file.write("x position: {}\n".format(Position_Message.linear.x))
            log_file.write(str(Position_Message.linear.x)+ '\n')
        with open("/home/administrator/ws_daniel/y_posData.txt", "a") as log_file:
            log_file.write(str(Position_Message.linear.y)+ '\n')
        with open("/home/administrator/ws_daniel/z_posData.txt", "a") as log_file:
            log_file.write(str(Position_Message.linear.z)+ '\n')
